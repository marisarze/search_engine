    ...
    for (int request_id=0;request_id<answers.size();request_id++){
        ...
        if (answers[request_id].size()==0)
            ...
            for (int doc_id=0;doc_id<answers[request_id].size();doc_id++){
                ...
                answerEntry["relevance"].add(pair);
            }
        }
...
//    ConverterJSON(std::string inFolder=""): folder(inFolder){};
//
...
//        json answersJSON = {"answers": {}};
//        for (int request_id=0;request_id<answers.size();request_id++){
//            std::string request_key = "request" + to_string(request_id+1);
//            json answerEntry = {};
//            if (answers[request_id].size()==0)
//                answerEntry["result"]="false";
//            else {
...
//                for (int doc_id=0;doc_id<answers[request_id].size();doc_id++){
//                    json pair = {};
//                    pair["docid"] = answers[request_id][docid]->first;
//                    pair["rank"] = answers[request_id][docid]->second;
//                    answerEntry["relevance"].add(pair);
//                }
//            }
//            answersJSON[request_key] = answerEntry;
